...
class MainController(MainViewProtocol):
    def __init__(self, root: ctk.CTk):
        ...
        self.shortcut_controller = ShortcutController(
            self.root, self.config_model
        )
        self.history_controller = HistoryController(
            self.root, self.translation_model
        )
        ...
        self.root.bind(
            "<<TranslationStarted>>",
            lambda e: self._handle_translation_started(),
        )
        self.root.bind(
            "<<TranslationStopped>>",
            lambda e: self._handle_translation_stopped(),
        )
    ...
    def _load_saved_settings(self):
        ...
        try:
            ...
            engine = self.config_model.get_config(
                "translation", "engine", "Gemini"
            )
            self.main_view.translation_engine.set(engine)

            # Load OCR engine
            ocr_engine = self.config_model.get_config(
            ...
            game_mode = self.config_model.get_config(
            ...
            self.main_view.opacity_var.set(
                opacity / 100
            )  # Convert from percentage

            # Load shortcut settings
            shortcuts_enabled = self.config_model.get_config(
    ...
    def _register_shortcuts(self):
        ...
        self.shortcut_controller.set_shortcut_handler(
            "<Control-space>", self._toggle_translation
        )

        # Select Region
        self.shortcut_controller.set_shortcut_handler(
            "<Control-r>", self.on_select_region
        )

        # Cycle Translation Engine
        self.shortcut_controller.set_shortcut_handler(
            "<Control-t>", self._cycle_translation_engine
        )

        # Cycle OCR Engine
        self.shortcut_controller.set_shortcut_handler(
            "<Control-o>", self._cycle_ocr_engine
        )
    ...
    def _cycle_translation_engine(self):
        ...
        try:
            next_engine = (
                self.translation_controller.cycle_translation_engine()
            )
            self.main_view.translation_engine.set(next_engine)
            self.main_view.show_toast(
                f"Translation engine changed to {next_engine}"
            )
        except Exception as e:
        ...
        try:
            if self.translation_controller.select_screen_region():
                self.main_view.update_region_status(
                    "✅ Region selected", ("#2B7539", "#1F5C2D")
                )
                self.main_view.enable_translation_button()
            else:
                self.main_view.update_region_status(
                    "❌ No region selected", ("#C42B2B", "#8B1F1F")
                )
    ...
    def on_change_opacity(self, value: float):
        ...
        try:
            ...
            self.config_model.update_config(
                "window", "opacity", opacity_percentage
            )
    ...
    def on_change_translation_engine(self, engine_name: str):
        ...
        try:
            ...
            is_translating = (
                hasattr(self.translation_controller, "is_translating")
                and self.translation_controller.is_translating
            )
            ...
            self.main_view.show_error(
                "Error", f"Failed to change translation engine: {e}"
            )
    ...
    def on_change_ocr_engine(self, engine_name: str):
        ...
        try:
            ...
            self.main_view.show_error(
                "Error", f"Failed to change OCR engine: {e}"
            )
    ...
    def on_change_source_language(self, language: str):
        ...
        try:
            self.config_model.update_config(
                "translation", "source_lang", language
            )
            self.main_view.show_toast(
                f"Source language changed to {
                    language.upper()}"
            )
        except Exception as e:
    ...
    def on_change_target_language(self, language: str):
        ...
        try:
            self.config_model.update_config(
                "translation", "target_lang", language
            )
            self.main_view.show_toast(
                f"Target language changed to {
                    language.upper()}"
            )
        except Exception as e:
    ...
    def _handle_translation_started(self):
        ...
        self.main_view.update_translation_status(
            "Translation: Active", ("#2B7539", "#1F5C2D")
        )
    ...
    def _handle_translation_stopped(self):
        ...
        self.main_view.update_translation_status(
            "Translation: Stopped",
            # Light gray and dark gray for normal/hover states
            ("#808080", "#666666"),
        )
