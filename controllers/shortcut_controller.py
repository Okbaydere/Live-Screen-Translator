...
class ShortcutController:
    ...
    SPECIAL_KEYS = {"space": "space", "r": "r", "t": "t", "o": "o", "h": "h"}

    def __init__(self, root: ctk.CTk, config_model: ConfigModel):
        self.root = root
        self.config_model = config_model
        # Store both formats for each shortcut
        # keyboard_format -> (handler, tkinter_format)
        self._shortcuts: Dict[str, Tuple[Callable, str]] = {}
        self._global_shortcuts_enabled = (
            False  # Start with global shortcuts disabled
        )
    ...
    def _parse_tkinter_format(tkinter_format: str) -> str:
        ...
        if not (
            tkinter_format.startswith("<") and tkinter_format.endswith(">")
        ):
    ...
    def _convert_modifier_keys(key_parts: list) -> list:
        ...
        return [
            ShortcutController.MODIFIER_MAP.get(part.lower(), part.lower())
    ...
    def _convert_special_keys(key_parts: list) -> list:
        ...
        return [
            ShortcutController.SPECIAL_KEYS.get(part.lower(), part)
            for part in key_parts
        ]
    ...
    @staticmethod
    def _handle_keyboard_error(
        operation: str, keyboard_format: str, error: Exception
    ) -> None:
        """Handle keyboard library errors"""
        error_msg = str(error)
        if "not found" in error_msg:
            logging.warning(
                f"Hotkey {keyboard_format} not found during {operation}"
            )
        else:
            logging.error(
                f"Keyboard error during {operation} for {keyboard_format}: {error}"
            )
    ...
    def set_shortcut_handler(self, tkinter_format: str, handler: Callable):
        ...
        try:
            ...
            logging.info(
                f"Registering shortcut: {tkinter_format} -> {keyboard_format}"
            )
            ...
            if keyboard_format in self._shortcuts:
                ...
                if self._global_shortcuts_enabled:
                    try:
                        ...
                        ShortcutController._handle_keyboard_error(
                            "remove_hotkey", keyboard_format, remove_err
                        )
            ...
            logging.error(
                f"Invalid shortcut format {tkinter_format}: {val_err}"
            )
        except Exception as setup_err:
            logging.error(
                f"Unexpected error setting shortcut {tkinter_format} ({keyboard_format}): {setup_err}"
            )
        ...
        try:
            ...
            for keyboard_format, (
                ...
                if enabled:
                    ...
                    logging.info(
                        f"Re-registered global shortcut: {keyboard_format}"
                    )
                else:
                    self.root.bind(tkinter_format, lambda event: handler())
                    logging.info(
                        f"Re-registered local shortcut: {tkinter_format}"
                    )
